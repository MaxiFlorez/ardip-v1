                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex justify-end items-center space-x-3">
                                            <a href="{{ route('personas.show', $persona) }}" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300" title="Ver Detalles">
                                                👁️
                                            </a>
                                            @can('operativo-escritura')
                                                <a href="{{ route('personas.edit', $persona) }}" class="text-warning-600 hover:text-warning-900 dark:text-warning-400 dark:hover:text-warning-300" title="Editar">
                                                    ✏️
                                                </a>
                                ...
                                @foreach($personas as $persona)
                                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                                        <div class="flex items-start mb-3">
                                            @if($persona->foto)
                                                <img src="{{ asset('storage/' . $persona->foto) }}" alt="{{ $persona->nombre_completo }}" class="w-16 h-16 rounded-full object-cover border border-gray-200 dark:border-gray-600">
                                            @else
                                                <div class="w-16 h-16 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-2xl text-gray-500 dark:text-gray-400">👤</div>
                                            @endif
                                            <div class="ml-3 flex-1">
                                                <h3 class="font-semibold text-gray-900 dark:text-gray-100">{{ $persona->apellido }}, {{ $persona->nombre }}</h3>
                                                <p class="text-sm text-gray-500 dark:text-gray-400">DNI: {{ $persona->dni }}</p>
                                                @if($persona->aliases->count() > 0)
                                                    <p class="text-sm text-gray-500 dark:text-gray-400">Alias: {{ $persona->aliases->pluck('alias')->join(', ') }}</p>
                                                @endif
                                                @if(!empty($persona->fecha_nacimiento))
                                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $persona->edad }} años ({{ optional($persona->fecha_nacimiento)->format('d/m/Y') }})</p>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="flex gap-2 flex-wrap">
                                            <a href="{{ route('personas.show', $persona) }}" class="flex-1 text-center px-3 py-2 bg-primary-600 text-white text-xs rounded hover:bg-primary-700 transition">
                                                👁️ Ver
                                            </a>
                                            @can('operativo-escritura')
                                                <a href="{{ route('personas.edit', $persona) }}" class="flex-1 text-center px-3 py-2 bg-warning-600 text-white text-xs rounded hover:bg-warning-700 transition">
                                                    ✏️ Editar
                                                </a>
                                                <form action="{{ route('personas.destroy', $persona) }}" method="POST" class="flex-1" onsubmit="return confirm('¿Confirma que desea eliminar esta persona?');">
                                                    @csrf
                                                    @method('DELETE')
