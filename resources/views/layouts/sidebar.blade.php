<div class="p-4">
    <div class="flex items-center justify-between mb-4 lg:hidden">
        <span class="text-xl font-bold text-white tracking-wide">ARDIP</span>
        <button id="sidebar-close-btn" class="inline-flex items-center justify-center p-2 rounded-md text-gray-100 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Cerrar menú">
    ...
    <a href="{{ route('dashboard') }}" class="hidden lg:flex items-center justify-center mb-6">
        <span class="text-2xl font-bold text-white tracking-wide">ARDIP</span>
    </a>

    <nav class="flex flex-col space-y-1">
        @php
            $linkBase = 'flex items-center gap-3 rounded px-3 py-2 text-sm font-medium cursor-pointer';
        @endphp

        <!-- Lista de Procedimientos -->
        @php $active = request()->routeIs('procedimientos.*'); @endphp
        <a href="{{ route('procedimientos.index') }}"
           class="{{ $linkBase }} {{ $active ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
        ...
        <!-- Nueva Carga -->
        @php $active = request()->routeIs('procedimientos.create'); @endphp
        ...
        <!-- Cerrar sesión en footer (opcional en sidebar) -->
        <form method="POST" action="{{ route('logout') }}" class="mt-4 border-t border-gray-700 pt-3">
            @csrf
            <button type="submit" class="w-full text-left {{ $linkBase }} text-red-200 hover:bg-gray-800 hover:text-red-100">
                <!-- Heroicon: Arrow Left On Rectangle -->
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
                    <path d="M12.75 3.75a.75.75 0 0 0-1.5 0v7.5H7.5a.75.75 0 0 0 0 1.5h3.75v7.5a.75.75 0 0 0 1.5 0v-7.5H16.5a.75.75 0 0 0 0-1.5h-3.75v-7.5Z"/>
                </svg>
                <span>{{ __('Salir') }}</span>
            </button>
        </form>
