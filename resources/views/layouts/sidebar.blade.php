<div class="p-4">
    ...
    <nav class="flex flex-col space-y-1">
        @php
            $linkBase = 'group flex items-center gap-3 rounded px-3 py-2 text-sm font-medium transition-colors';
        @endphp

        <!-- Lista de Procedimientos (Primero) -->
        @php $active = request()->routeIs('procedimientos.*'); @endphp
        <a href="{{ route('procedimientos.index') }}"
           class="{{ $linkBase }} {{ $active ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
